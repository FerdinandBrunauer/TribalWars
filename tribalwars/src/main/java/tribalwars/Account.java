...
public class Account implements Runnable {
							...
							while (currentVillage.farmPossible(vorlagen) && farmen.size() > 0) {
								...
								for (FarmVorlage vorlage : vorlagen) {
									if (currentVillage.canFarm(vorlage)) {
										document = browser.POST("http://" + welt + weltNummer + ".die-staemme.de/game.php?village=" + currentVillage.getId() + "&try=confirm&screen=place",
												hashName + "=" + hashValue + "&template_id=&spear=" + vorlage.getSpeertraeger() + "&sword=" + vorlage.getSchwertkaempfer() + "&axe=" + vorlage.getAxtkaempfer() + "&archer=" + vorlage.getBogenschuetzen() + "&spy=" + vorlage.getSpaeher() + "&light="
														+ vorlage.getLeichteKavallerie() + "&marcher=" + vorlage.getBerittenerBogenschuetze() + "&heavy=" + vorlage.getSchwereKavallerie() + "&ram=" + vorlage.getRammboecke() + "&catapult=" + vorlage.getKatapult() + "&snob=&x=" + farmen.get(0).x
														+ "&y=" + farmen.get(0).y + "&target_type=coord&input=" + farmen.get(0).x + "%7C" + farmen.get(0).y + "&attack=Angreifen");
										...
										document = browser.POST(
												"http://" + welt + weltNummer + ".die-staemme.de/game.php?village=" + currentVillage.getId() + "&action=command&h=" + hWert + "&screen=place",
												"attack=true&ch=" + chWert + "&x=" + farmen.get(0).x + "&y=" + farmen.get(0).y + "&action_id=" + action_ID + "&spear=" + vorlage.getSpeertraeger() + "&sword=" + vorlage.getSchwertkaempfer() + "&axe=" + vorlage.getAxtkaempfer() + "&archer="
														+ vorlage.getBogenschuetzen() + "&spy=" + vorlage.getSpaeher() + "&light=" + vorlage.getLeichteKavallerie() + "&marcher=" + vorlage.getBerittenerBogenschuetze() + "&heavy=" + vorlage.getSchwereKavallerie() + "&ram=" + vorlage.getRammboecke()
														+ "&catapult=" + vorlage.getKatapult() + "&snob=0");
										Thread.sleep(1596);
	...
	public boolean login() throws IOException, CaptchaException, SessionException {
	...
	public void refreshVillages() throws IOException, CaptchaException, SessionException {
